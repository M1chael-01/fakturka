...
const invoices = await reader.fetchedData("PF"); // nebo jiný typ
...
const getData = async () => {
  try {
    const res = await fetch(`${GetBackendURL()}/invoice/getInvoices?type=PF`, {
      method: "GET",
      credentials: "include",
      headers: {
    ...
    console.log("✅ Načtené faktury:", data);
                ...
                filteredInvoices.map((invoice) => (
                  <tr key={invoice.id}>
                    ...
                    <td>
                      {invoice.file ? (
                        <a
                          href={URL.createObjectURL(invoice.file)}
                          target="_blank"
                          rel="noreferrer"
                        >
                          {invoice.file.name.endsWith(".pdf") ? (
                            <FaFilePdf />
                          ) : (
                            <FaFileWord />
                          )}{" "}
