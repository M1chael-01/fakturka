...
import ReceivedInvoiceEditorA4 from "./Components/User/Pages/ReceivedInvoiceEditorA4";
...
const isPlanPaid = async () => {
  try {
    const res = await fetch(`${GetBackendURL()}/payment/isPaid`, {
      method: "GET",
      credentials: "include", // důležité pro cookies s JWT
    });

    if (!res.ok) {
      console.error("Nepodařilo se ověřit stav platby.");
      return false;
    }
    ...
    console.error("Chyba při kontrole placeného plánu:", err);
    return false;
  }
};
...
/* const isPlanPaid = async () => {
 ...
 const handleCheckPayment = async () => {
    const paid = await isPlanPaid();
    console.log(paid);
    console.log(publicPaths)
      const currentPlanId = Number(localStorage.getItem("planID")) || 0;
    if (!paid && !publicPaths.includes(location.pathname) && currentPlanId !== 0) {
      
      navigate("/platba-predplatne");
  ...
  return (
    <Routes>
      ...
      {/* ReceivedInvoiceEditorA4 */}
            <Route path="/faktury-prijate/nova" element={<ReceivedInvoiceEditorA4 />} />
